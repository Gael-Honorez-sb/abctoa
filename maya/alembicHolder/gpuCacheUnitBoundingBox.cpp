...
MMatrix UnitBoundingBox::boundingBoxMatrix(const Imath::Box3d& boundingBox)
{
    const MPoint offset = MPoint(boundingBox.min.x, boundingBox.min.y, boundingBox.min.z);
    const auto diag = boundingBox.max - boundingBox.min;
    const MPoint scale = MPoint(diag.x / 2.0f,
                                diag.y / 2.0f,
                                diag.z / 2.0f);
