...
namespace {
    ...
    Alembic::Abc::IObject getAlembicTopObject(const std::string& file_name)
    {
        Alembic::AbcCoreFactory::IFactory factory;
        factory.setOgawaNumStreams(16);
        auto archive = factory.getArchive(file_name);
        if (!archive.valid())
...
AlembicScene::AlembicScene(const AlembicSceneKey& scene_key)
    : m_hierarchy(getAlembicTopObject(scene_key.file_ref.path()), scene_key.root_path)
